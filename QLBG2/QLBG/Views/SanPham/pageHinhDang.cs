...
{
    public partial class pageHinhDang : UserControl
    {
        private HinhDangDAL hinhDangDAL;

        public pageHinhDang()
        {
            InitializeComponent();
            hinhDangDAL = new HinhDangDAL();
            LoadData();
        ...
        public void LoadData()
        {
            ...
            lbSoLuong.Text = hinhDangList.Count.ToString();
            dgvDanhSach.Rows.Clear();
            foreach (var hinhDang in hinhDangList)
            {
                dgvDanhSach.Rows.Add(hinhDang.MaHinhDang, hinhDang.TenHinhDang);
            }
        }
                ...
                if (frmThuocTinh.ShowDialog() == DialogResult.OK)
                {
                    ...
                    switch (frmThuocTinh.mode)
                    {
                        case frmThuocTinhSanPham.Mode.Edit:
                            ...
                            var updatedHinhDang = new HinhDangDTO
                            {
                                MaHinhDang = Convert.ToInt32(row.Cells[0].Value),
                                TenHinhDang = result.Cells[1].Value.ToString()
                            };
                            if (!hinhDangDAL.UpdateHinhDang(updatedHinhDang))
                            {
                                MessageBox.Show("Cập nhật thất bại.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            break;
                        case frmThuocTinhSanPham.Mode.Delete:
                            int maHinhDang = Convert.ToInt32(row.Cells[0].Value);
                            if (hinhDangDAL.DeleteHinhDang(maHinhDang))
                            {
                                LoadData();
                            }
                            else
        ...
        private void btnTao_Click(object sender, EventArgs e)
        {
            ...
            using (frmThuocTinhSanPham form = new frmThuocTinhSanPham(newRow, frmThuocTinhSanPham.Mode.Add))
            {
                ...
                {
                    newRow = form.row;
                    var newHinhDang = new HinhDangDTO
                    {
                        MaHinhDang = 0,
                        TenHinhDang = newRow.Cells[1].Value.ToString()
                    };
                    if (hinhDangDAL.InsertHinhDang(newHinhDang))
                    {
                        LoadData();
                    }
                    else
        ...
        private void btnXuat_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog saveFileDialog = new SaveFileDialog())
            {
                ...
                saveFileDialog.FileName = "HinhDang.xlsx";

                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    using (var workbook = new XLWorkbook())
                    {
                        var worksheet = workbook.Worksheets.Add("HinhDang");

                        worksheet.Cell(1, 1).Value = "MaHinhDang";
                        worksheet.Cell(1, 2).Value = "TenHinhDang";
        ...
        private void btnTim_Click(object sender, EventArgs e)
        {
            ...
            var filteredList = hinhDangList.FindAll(hinhDang =>
                hinhDang.TenHinhDang.ToLower().Contains(searchText) ||
                hinhDang.MaHinhDang.ToString().Contains(searchText));

            dgvDanhSach.Rows.Clear();
            foreach (var hinhDang in filteredList)
            {
                dgvDanhSach.Rows.Add(hinhDang.MaHinhDang, hinhDang.TenHinhDang);
            }
