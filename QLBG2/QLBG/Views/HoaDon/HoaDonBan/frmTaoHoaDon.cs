...
namespace QLBG.Views.HoaDon.HoaDonBan
{
    public partial class frmTaoHoaDon : Form
    {
        ...
        private void LoadKH()
        {
            DataTable khachHang = khachHangDAL.GetAllKhachHang();
            foreach (DataRow row in khachHang.Rows)
            {
                dgvTim.Rows.Add(row["MaKhach"], row["TenKhach"], row["DiaChi"], row["DienThoai"]);
            }
        }
        ...
        private void ProductItem_OnProductItemClick(object sender, EventArgs e)
        {
            ...
            if (dgvSanPham.Rows.Count > 0)
            {
                ...
                {
                    if (row.Cells["MaHang"].Value.ToString() == maSP.ToString())
                    {
        ...
        private void InitDGV()
        {
            ...
            dgvTim.Columns.Add(new DataGridViewTextBoxColumn { Name = "MaKhach", HeaderText = "Mã Khách Hàng" });
            dgvTim.Columns.Add(new DataGridViewTextBoxColumn { Name = "TenKhach", HeaderText = "Tên Khách Hàng" });
            ...
            dgvDanhSach.Columns.Add(new DataGridViewTextBoxColumn { Name = "MaKhach", HeaderText = "Mã Khách" });
            ...
            dgvSanPham.Columns.Add(new DataGridViewTextBoxColumn { Name = "MaHang", HeaderText = "Mã sản phẩm", ReadOnly = true });
            dgvSanPham.Columns.Add(new DataGridViewTextBoxColumn { Name = "TenHangHoa", HeaderText = "Tên sản phẩm", ReadOnly = true });
        ...
        private void dgvTim_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                dgvDanhSach.Rows.Add(dgvTim.Rows[e.RowIndex].Cells["MaKhach"].Value, dgvTim.Rows[e.RowIndex].Cells["TenKhach"].Value, dgvTim.Rows[e.RowIndex].Cells["DiaChi"].Value, dgvTim.Rows[e.RowIndex].Cells["DienThoai"].Value);
            }
        }

        private void btnTim_Click(object sender, EventArgs e)
        {
            string tenKhach = txtTim.Text;
            DataTable dt = khachHangDAL.GetAllKhachHang();
            dgvTim.Rows.Clear();
            foreach (DataRow row in dt.Rows)
            {
                if (row["TenKhach"].ToString().Contains(tenKhach))
                {
                    dgvTim.Rows.Add(row["MaKhach"], row["TenKhach"], row["DiaChi"], row["DienThoai"]);
                }
            }
        ...
        private void btnTao_Click(object sender, EventArgs e)
        {
            ...
            var maKH = dgvDanhSach.Rows[0].Cells["MaKhach"].Value;
            ...
            if (dgvSanPham.Rows.Count > 0)
            {
                ...
                foreach (DataGridViewRow row in dgvSanPham.Rows)
                {
                    var maSP = row.Cells["MaHang"].Value;
                    ...
                    if (sp.SoLuong < 0)
                    { 
                        MessageBox.Show("Số lượng sản phẩm " + maSP + " " + row.Cells["TenHangHoa"] + " không đủ");
                        return;
                    }
                    listSP.Add(sp);
                ...
                int soHDB = HoaDonDAL.ThemHoaDonBan(int.Parse(maNV.ToString()), int.Parse(maKH.ToString()), ngayNhap, tongTien);
                if (soHDB < 0)
                {
                ...
                foreach (DataGridViewRow row in dgvSanPham.Rows)
                {
                    ...
                    if (!HoaDonDAL.ThemChiTietHoaDon(soHDB, int.Parse(maSP.ToString()), sl, decimal.Parse(giamGia.ToString()), decimal.Parse(thanhTien.ToString())))
                    {
